...
export default function PinnedRepos(props: { pinnedRepos: PinnedRepos }) {
  return (
    <section className="flex flex-col w-full gap-6 mb-10 prose prose-a:no-underline">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 w-full gap-6">
        {props.pinnedRepos.map((repo, index) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="bg-gradient-to-r from-[#FDE68A] via-[#FCA5A5] to-[#FECACA] p-1 gap-4 rounded-lg shadow-lg dark:shadow-zinc-200/10 shadow-zinc-800/10 hover:shadow-xl hover:scale-[103%] dark:hover:shadow-zinc-200/10 hover:shadow-zinc-800/10 duration-300">
      ...
      <a
        href="https://github.com/bhaveek424?tab=repositories"
        target="_blank"
        rel="noreferrer"
