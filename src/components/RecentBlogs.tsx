...
export default function RecentBlogs(props: { recentBlogs: RecentBlogs }) {
  return (
    <section className="flex flex-col w-full gap-6 mb-20 prose prose-a:no-underline">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 w-full gap-6">
        {props.recentBlogs.map((post) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="bg-gradient-to-r from-[#D8B4FE] to-[#818CF8] w-full p-1 rounded-lg shadow-lg dark:shadow-zinc-200/10 shadow-zinc-800/10 hover:shadow-xl hover:scale-[103%] dark:hover:shadow-zinc-200/10 hover:shadow-zinc-800/10 duration-300">
      ...
      <a
        href="https://bhaveek.hashnode.dev/"
        target="_blank"
        rel="noreferrer"
